...
function drawTile(x, y, sol, color) {
    ...
    setAttributes(tile,{"x": x + gridStart, "y": y + gridStart, "width": tileSize, "height": tileSize, "sol": sol, "curr": 0, "flag": 0,"fill": color});
...
function islandCalc(a) {
    ...
    for (var i=0;i<a.length;i++) {   
        ...
        if (a[i] == false && n > 0 || i == a.length-1 && n > 0) {
            m.push(n);
            n = 0;
        }
    }
...
function mouseClick() {
    ...
    if (event.button === 2) toggleFlag();
}

function mouseDrag() {
    if (mouseState === true && event.button === 0) toggleTile();
    if (mouseState === true && event.button === 2) toggleFlag();
}
